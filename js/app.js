...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    //set innerHTML of product class
    div.innerHTML =
      `<div class="single-product card border-secondary border-2 bg-white m-2 p-2 h-100">
          <img src='${product.image}' class="image-size mx-auto rounded"></img>
          <div class="card-body text-center">
            <h4 class="card-title fw-semibold fs-5 mb-3">${product.title}</h4>
            <h5 class="card-text fs-6">Category: ${product.category}</h5>
            <h3 class="card-text fw-semibold fs-5 mt-4">Price: $ <span class="text-danger fw-bold">${product.price}</span></h3><br>
            <h4 class="card-text fs-5"><i class="fas fa-star"></i> ${product.rating.rate}<i class="ps-4 fas fa-user"></i> ${product.rating.count}</h4>
          </div>
          <div class="card-footer text-center bg-white border-0 rounded-pill d-flex align-items-center justify-content-center">
              <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now rounded-pill btn btn-success me-2"><i class="fas fa-shopping-cart"></i> Add to Cart</button>
              <button onclick="detailsButton()" class="rounded-pill btn btn-secondary text-white">Details</button>
          </div>
      </div>`;
...
const updateTaxAndCharge = () => {
  ...
  //if product price is more than $200 then delivery charge=$30 and total-tax=20%
  if (priceConverted > 200) {
    setInnerText("delivery-charge", 30);
    setInnerText("total-tax", priceConverted * 0.2);
  }
  //if product price is more than $400 then delivery charge=$50 and total-tax=30%
  if (priceConverted > 400) {
    setInnerText("delivery-charge", 50);
    setInnerText("total-tax", priceConverted * 0.3);
  }
  //if product price is more than $500 then delivery charge=$60 and total-tax=40%
...
const updateTotal = () => {
  //calculate price,delivery charge and total-tax
  const grandTotal =
